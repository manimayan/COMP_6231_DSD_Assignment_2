...
public class LavalServer extends DCMSInterfacePOA {
	 ...
	 */
	public void serverConnection(int port, LavalServer lavalServerArg) {

		/*
		 * LavalServer lavalServer = lavalServerArg; Registry registry2 =
		 * LocateRegistry.createRegistry(9992); registry2.bind("Laval", lavalServer);
		 */
	...
	public synchronized String createTRecord(String recordId, String firstName, String lastName, String address,
		...
		if (this.mainServer.findDuplicateRecordInmap(recordId, this.teacherStudentRecords)) {
			...
			resultMsg = this.mainServer.persistTeacherRecordInMap(teacherRecord, this.teacherStudentRecords)
					? "Teacher record " + teacherRecord.getRecordID() + outputType + "Laval location with name : "
							+ firstName + " " + lastName
	...
	public synchronized String createSRecord(String recordId, String firstName, String lastName,
		...
		if (this.mainServer.findDuplicateRecordInmap(recordId, this.teacherStudentRecords)) {
			...
			resultMsg = this.mainServer.persistStudentRecordInMap(studentRecord, this.teacherStudentRecords)
					? "Student record " + studentRecord.getRecordID() + outputType + "Laval location with name : "
							+ firstName + " " + lastName
	...
	public String findRecord(String recordID) {
		return this.mainServer.findAndRemoveRecordInmap(recordID, LogWriter.LAVAL, this.teacherStudentRecords);
	}
